...
* **🔒 Autenticación Estricta (Case-Sensitive):** Se modificó la consulta SQL de validación de inicio de sesión en [conexion.py](file:///home/favio/Proyectos/cyber-reinoso/conexion.py) para utilizar `COLLATE Latin1_General_CS_AS` al contrastar el usuario y la contraseña. Esto corrige la insensibilidad predeterminada de SQL Server, garantizando que el acceso con versiones en mayúsculas (como `ADMIN`) sea rechazado si en la base de datos solo existe el registro en minúsculas (`admin`), aumentando la robustez y seguridad del acceso.
